...
async def random_swing_check(bot):
    # 1/100 chance for the swing to happen
        swing_chance = 1 #random.randint(1, 100)
...
async def send_swing(bot, user_id, swing_type, channel_type):
    ...
    if channel_type == "info_channel":
        ...
        channel = bot.fetch_channel(user.dm_channel.id) if user.dm_channel else await user.create_dm()
    
    # send the swing
    if swing_type == "insult":
        await channel.send(content=await send_insult_swing())
    elif swing_type == "neutral":
        await channel.send(content=await send_neutral_swing())
