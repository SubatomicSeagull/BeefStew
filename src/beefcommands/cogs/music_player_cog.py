...
class MusicPlayerCog(commands.Cog):
    ...
    async def mp_play(self, ctx: commands.Context, *args):
        print(f"> \033[32m{ctx.author.name} used /play\033[0m")
        try:
            await music_player.play(ctx.author, ctx.voice_client, ctx.channel, *args)
    ...
    async def mp_pause(self, ctx):
        ...
        await music_player.pause(ctx.voice_client)
    ...
    async def mp_queue_list(self, ctx):
        ...
        await music_player.list(ctx.channel)
    ...
    async def mp_loop_queue(self, ctx):
        ...
        await music_player.loop(ctx.channel)
