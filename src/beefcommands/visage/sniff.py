...
async def sniff_overlay(victim: discord.Member):
    ...
    x = (overlay_width - base_width) // 1 - 0
    y = (overlay_height - base_height) // 1 - 0
...
async def sniff_user(interaction: discord.Interaction, victim: discord.Member):
    ...
    try:
        ...
        await interaction.followup.send(content= f"sniff", file=discord.File(fp=drain_pfp, filename=f"{victim.name}_sniff.png"))
